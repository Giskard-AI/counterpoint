...
class TestPromptsManager:
    ...
    @patch("counterpoint.templates.prompts_manager.importlib.import_module")
    async def test_render_template_with_unresolvable_submodule_namespace(
        self, mock_import
    ):
        manager = PromptsManager()
        ...
        with pytest.raises(
            ValueError,
            match="Prompt source lidar.probes not registered and package not found",
        ):
    ...
    def test_thread_safety_concurrent_registration(self):
        ...
        for i in range(10):
            thread = threading.Thread(
                target=register_source, args=(f"namespace_{i}", f"/path/to/source_{i}")
            )
        ...
        for i in range(10):
            ...
            assert manager.prompts_sources[f"namespace_{i}"] == Path(
                f"/path/to/source_{i}"
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "Package test_package not found",
            ),
            (
                PermissionError("Permission denied"),
                "Error resolving package test_package: Permission denied",
            ),
            (
                FileNotFoundError("File not found"),
                "Error resolving package test_package: File not found",
            ),
            (
                RuntimeError("Runtime error"),
                "Error resolving package test_package: Runtime error",
            ),
        ],
    )
    @patch("counterpoint.templates.prompts_manager.importlib.import_module")
    def test_warning_on_namespace_resolution_errors(
        self, mock_import, exception, expected_warning
    ):
    ...
    async def test_render_template_explicit_registration_precedence(self):
        ...
        with patch(
            "counterpoint.templates.prompts_manager.create_message_environment"
        ) as mock_env:
    ...
    async def test_valid_namespace_formats(self, namespace):
        ...
        assert "Invalid namespace format" not in str(exc_info.value), (
            f"Namespace {namespace} should be valid format"
        )
        assert "not registered and package not found" in str(exc_info.value)

    @pytest.mark.parametrize(
        "source_path,namespace,expected_path",
        [
    ...
    def test_path_normalization_in_registration(
        self, source_path, namespace, expected_path
    ):
